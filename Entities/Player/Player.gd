...
@onready var engine_sound_player: AudioStreamPlayer = $EngineSoundPlayer
...
# --- MODIFIED: Orbiting State ---
enum State { FLYING, ORBITING, DOCKING } # <-- ADDED DOCKING
...
func _ready() -> void:
	...
	# --- MODIFIED: Removed intro complete signal ---
	# EventBus.sector_intro_complete.connect(_on_sector_intro_complete) # <-- REMOVED
...
func _physics_process(_delta: float) -> void:
	...
	if is_instance_valid(engine_sound_player):
		if thrust_pressed or reverse_pressed:
			if not engine_sound_player.is_playing():
				engine_sound_player.play()
		else:
			if engine_sound_player.is_playing():
...
# --- NEW FUNCTION ---
func set_state_docking() -> void:
	_state = State.DOCKING
func set_state_orbiting() -> void:
	_state = State.ORBITING
func get_orbiting_planet() -> Node2D:
	return _orbiting_planet
# --- MODIFIED: This is now called by main.gd ---
func _on_sector_intro_complete() -> void:
	# Check state
	if _state == State.DOCKING:
		# We are docked, do not re-enable physics
		return
